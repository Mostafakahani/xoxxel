  ...
  return (
    <div>
      <Typography variant="h6" style={{ marginTop: "15px" }}>
        لیست اسلایدر کمپانی ها
      </Typography>
      {rows.length === 0 ? (
        <Typography my={2}>هیچ کمپانی وجود ندارد</Typography>
      ) : (
        <div>
          {rows.map((row) => (
            <Accordion
            ...
            >
              <AccordionSummary aria-controls={`panel${row.id}d-content`} id={`panel${row.id}d-header`}>
                <Typography sx={{ color: "#2C7EFA", my: "10px", fontSize: "16px", width: "100%", containerType: "inline-size", overflow: "hidden", textOverflow: "ellipsis" }}>{row.title}</Typography>
              </AccordionSummary>
              <AccordionDetails>
...
export default CompaniesSliderList;
