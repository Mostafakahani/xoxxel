...
import TopUpPopularGamesForm from "Components/Common/HomePageSteps/TopUpPopularGames/TopUpPopularGamesForm";
import TopUpPopularGamesList from "Components/Common/HomePageSteps/TopUpPopularGames/TopUpPopularGamesList";
...
const ChangeTopUpPopularGames = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                const responseLevels = await axios.get(
                    `${ServerURL.url}/admin/sliders/top-up/popular-games/list`,
                    config
                );
    ...
    const handleAddRow = () => {
        ...
        if (input1.trim() === "" || input2.trim() === "" || selectedFileItem.length === 0) {
        ...
        if (!isDuplicate) {
            if (rows.length < 3) {
                // setRows([...rows, { id: Date.now(), title: input3, link: input1, deliveryType: input2, save: input4, btn: input5, id_storage: selectedFileItem }]);
                setRows([...rows, { id: Date.now(), title: input2, link: input1, id_storage: selectedFileItem }]);
                setSelectedFileItem([]);
                setInput1('');
                setInput2('');
                setInput3("");
    ...
    const handleDeleteRow = async (id) => {
        ...
        //     const response = await axios.post(`${ServerURL.url}/admin/sliders/giftcart/item/delete`, data, config);
    ...
    const handleSubmit = async () => {
        ...
        if (!isDuplicate) {
            ...
            try {
                ...
                const responseSend = await axios.post(`${ServerURL.url}/admin/sliders/top-up/popular-games/create`, dataSend, config);
                setSelectedFileItem([]);
                setInput1('')
                setRowsTemp([])
                setCount(count + 1)
                toast.success("با موفقیت اضافه شد");
                toast.error(responseSend.data.message[0] === "link must be a URL address" ? 'آدرس معتبر وارد کنید' : 'مشکلی پیش آمده است')
            } catch (error) {
                if (error?.response) {
                    toast.error(error?.response?.data?.message[0] === "link must be a URL address" ? 'آدرس معتبر وارد کنید' : 'مشکلی پیش آمده است', error.response.data)
                }
...
export default ChangeTopUpPopularGames;
